...
export default function ProjectMediaUploadForm({ action, projectId }) {
    ...
    <form ref={formRef} action={formAction} className="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="grid gap-1">
          <label htmlFor={`slot-${projectId}`} className="text-xs font-semibold uppercase tracking-wide text-slate-600">
            Slot
          </label>
          <select
            id={`slot-${projectId}`}
        ...
        <div className="grid gap-1">
          <label htmlFor={`orderStart-${projectId}`} className="text-xs font-semibold uppercase tracking-wide text-slate-600">
            Start Order
          </label>
          <input
            id={`orderStart-${projectId}`}
            name="orderStart"
            type="number"
            min={1}
            defaultValue={1}
      ...
      <div className="grid gap-2">
        <label htmlFor={`images-${projectId}`} className="text-xs font-semibold uppercase tracking-wide text-slate-600">
          Images
        </label>
